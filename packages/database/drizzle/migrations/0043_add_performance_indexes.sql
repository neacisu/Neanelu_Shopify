...
CREATE INDEX IF NOT EXISTS idx_products_shop_status_updated 
  ON shopify_products(shop_id, status, updated_at DESC);

-- Variants: low inventory lookup
CREATE INDEX IF NOT EXISTS idx_variants_shop_inventory_low 
...
CREATE INDEX IF NOT EXISTS idx_prod_embeddings_type_product 
  ON prod_embeddings(embedding_type, product_id);

-- Shop embeddings: shop filter
CREATE INDEX IF NOT EXISTS idx_shop_embeddings_shop_type 
...
CREATE INDEX IF NOT EXISTS idx_bulk_runs_shop_status_created 
...
CREATE INDEX IF NOT EXISTS idx_prod_master_quality_level 
  ON prod_master(data_quality_level);

-- PIM master: needs review queue
CREATE INDEX IF NOT EXISTS idx_prod_master_needs_review 
  ON prod_master(needs_review, updated_at DESC) 
  WHERE needs_review = true;

-- PIM master: brand lookup
CREATE INDEX IF NOT EXISTS idx_prod_master_brand 
...
CREATE INDEX IF NOT EXISTS idx_products_metafields_gin 
  ON shopify_products USING GIN(metafields jsonb_path_ops);

-- Variants metafields
CREATE INDEX IF NOT EXISTS idx_variants_metafields_gin 
...
-- PIM specs
CREATE INDEX IF NOT EXISTS idx_prod_specs_gin 
  ON prod_specs_normalized USING GIN(specs jsonb_path_ops);

-- PIM provenance
CREATE INDEX IF NOT EXISTS idx_prod_specs_provenance_gin 
