...
-- ============================================
-- UPGRADE: Migrate from vector(1536) to vector(2000)
...
-- ============================================
-- 3. MIGRATE VECTOR DIMENSION FROM 1536 TO 2000
-- ============================================
...
BEGIN
    ...
    IF deleted_count > 0 THEN
        ...
        RAISE NOTICE '⚠️ Deleted % existing embeddings (incompatible with new 2000 dimensions)', deleted_count;
    ELSE
...
-- Step 3.3: Alter embedding column to vector(2000)
ALTER TABLE prod_embeddings 
    ALTER COLUMN embedding TYPE vector(2000);

-- Step 3.4: Update dimensions default
ALTER TABLE prod_embeddings 
    ALTER COLUMN dimensions SET DEFAULT 2000;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE prod_embeddings 
            ...
            CHECK (dimensions = 2000);
        RAISE NOTICE '✅ Added CHECK constraint for dimensions = 2000';
    ELSE
...
-- Recreate HNSW index for vector(2000)
...
COMMENT ON FUNCTION find_similar_products(vector(2000), float, int) IS 
    'Find similar products using cosine similarity on 2000-dim embeddings (HNSW compatible)';
...
COMMENT ON FUNCTION find_similar_shop_products(uuid, vector(2000), float, int) IS 
    'Find similar shop products using cosine similarity on 2000-dim embeddings (RLS-aware, HNSW compatible)';
...
BEGIN
    ...
    IF idx_count >= 3 THEN
        RAISE NOTICE '✅ HNSW indexes verified: % indexes with vector(2000)', idx_count;
    ELSE
    ...
    RAISE NOTICE 'Run the backfill job to regenerate embeddings with 2000 dimensions.';
    RAISE NOTICE '============================================';
END $$;
