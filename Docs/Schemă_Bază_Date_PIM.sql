...
-- Enable UUIDv7 extension if not native (or pgcrypto)
...
-- Activare extensie pentru Vector Search (necesară pentru Attribute Matching)
...
CREATE TABLE prod_taxonomy (
    id UUID PRIMARY KEY, -- Changed UUIDv7 to UUID for compatibility with uuid-ossp
...
CREATE TABLE prod_attr_definitions (
    id UUID PRIMARY KEY, -- Changed UUIDv7 to UUID
...
CREATE TABLE prod_master (
    ...
    taxonomy_id UUID REFERENCES prod_taxonomy(id), -- Changed UUIDv7 to UUID
    created_at TIMESTAMPTZ DEFAULT NOW()
);
