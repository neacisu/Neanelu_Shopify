...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Patch OpenBao KV for Neanelu staging API (Shopify keys + APP_HOST) without printing secrets.\n"
            "Writes only to: secret/neanelu/staging/api"
        )
    )
    ...
    ap.add_argument(
        "--staging-env-path",
        required=True,
        help="Local env file containing SHOPIFY_API_KEY, SHOPIFY_API_SECRET, APP_HOST (staging).",
    )
    args = ap.parse_args()
    ...
    st = load_env_file(Path(args.staging_env_path))
    patch = {
        "shopify_api_key": required(st, "SHOPIFY_API_KEY"),
        "shopify_api_secret": required(st, "SHOPIFY_API_SECRET"),
        "app_host": required(st, "APP_HOST"),
        "updated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "note": "patched staging shopify keys + app_host",
    }
    ...
    # Do not print values; only acknowledge keys updated.
