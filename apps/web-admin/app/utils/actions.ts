import type { ActionFunctionArgs, UNSAFE_DataWithResponseInit } from 'react-router-dom';
...
// React Router actions can return a redirect `Response`, but `useActionData()` / fetchers
// should be typed to the JSON branch. Exclude `Response` to keep route typing ergonomic.
type AwaitedActionReturn<TAction> = TAction extends (...args: infer _Args) => infer R
  ? Awaited<R>
  : never;

export type ActionData<TAction> =
  AwaitedActionReturn<TAction> extends infer R
    ? R extends Response
      ? never
      : R extends UNSAFE_DataWithResponseInit<infer U>
        ? U
        : R
    : never;
 ...
 */
export function withShopifyQueryRedirect(args: { request: Request }, to: string): Response {
  // The app runs under a router `basename` (`/app`). Loaders/actions often see URLs like
